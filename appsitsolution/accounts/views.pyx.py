...
def user_login(request):
    if request.method == 'POST':
        if 'mobile' in request.POST and 'otp' in request.POST:
            ...
            email = request.POST.get('username')                                          # getting username from the user
...
def new_reg_user_login(request):
    ...
    try:
        check_verified = request.user.user_check.check_mobile
    except:
        pass
    check_verified = False
    if check_verified:
...
def new_sign_up(request):
    ...
    #setting a cookie
        ...
        response.set_cookie('position', position)
        return response
                                ...
                                if addres_query.save:
                                    ...
                                    if post.save:
                                        ...
                                        if bank_form.is_valid():
                                            ...
                                            res =  Bank_proof_api(request,post_bank_ifcs,post_bank_account_number)
...
# <-----------------------------------------------------------End email send message --------------------------------------------------------------------------------------------->
    ...
    if 'refid' in request.GET and 'position' in request.GET:
        refid = request.GET['refid']
        position = request.GET['position']
        context={'form':kyc, 'bank_form':bank_form,'title':'Registration','refid':refid,'position':position,'state':state}
    else:
        context={'form':kyc, 'bank_form':bank_form,'title':'Registration','state':state}
    # return render(request, 'registration1.html', context)
...
def logoutUser(request):
    ...
    cart_id = request.session['cart_id']
    for i in cart_items:
        item = CartItem(cart_id = cart_id,price = i.price,discount_price=i.discount_price,business_value = i.business_value,
                        point_value = i.point_value,quantity = i.quantity,in_stock = i.in_stock,product = i.product)
        item.save()
...
def edit_profile(request):
    ...
    if request.method == 'POST':
        ...
        kyc_pan_number = request.POST.get('pan_number',None)
        kyc_pan_file = request.FILES.get('pan_file', None)
        kyc_id_proof_type = request.POST.get('id_proof_type',None)
        kyc_id_proof_file = request.FILES.get('id_proof_file',None)
        kyc_address_proof_type = request.POST.get('address_proof_type',None)
        kyc_address_proof_file = request.FILES.get('address_proof_file',None)
        bank_distributors_name_in_bank_account = request.POST.get('distributors_name_in_bank_account',None)
        bank_bank_name = request.POST.get('bank_name',None)
        bank_account_number = request.POST.get('account_number',None)
        bank_ifsc_code = request.POST.get('ifsc_code',None)
        bank_branch_name = request.POST.get('branch_name',None)
        bank_cheque_photo = request.FILES.get('cheque_photo',None)
        ...
        if check_user_qs.check_distributors_name_in_bank_account:
            bank_distributors_name_in_bank_account = bank_qs.distributors_name_in_bank_account
        ...
        bank_update = BankAccountDetails.objects.filter(pk=bank_qs.pk).update(distributors_name_in_bank_account=bank_distributors_name_in_bank_account,
                                                                      bank_name=bank_bank_name,
        ...
        try:
            if kyc_qs:
                ...
                if kyc_form.is_valid():
                    post = kyc_form.save(commit=False)
                    post.kyc_user= request.user
                    pan_number = post.pan_number
                    try:
                        img_path = post.id_proof_file.path
                    except:
                        img_path = None
                ...
                kyc_form = Kyc.objects.filter(kyc_user = user).update(pan_number = kyc_pan_number,pan_file=post.pan_file,id_proof_type = kyc_id_proof_type,
                                                                  address_proof_type = kyc_address_proof_type)
                
                img_path = post.pan_file.path
                response_kyc = Pan_verification(request,img_path,kyc_pan_number)
                if response_kyc == 400:
                    messages.error(request,"Images not present in Identity, Update now")
                    return redirect("edit_profile")
                try:
                    ...
                    if response_kyc["verified"] == "True":
                        ...
                        post.save()
                        messages.success(request, "Congrats KYC verified")
                request.POST.get('pan_number',None)
            else:
                form = KycForm(request.POST, request.FILES)
                if form.is_valid():
                    dataform = form.save(commit=False)
                    dataform.kyc_user = user
                    pan_number = dataform.pan_number
                    try:
                        img_path = dataform.pan_file.path
                    except:
                        img_path = None  
                    
                    pan_name = post.pan_name

                    # kyc_form = Kyc.objects.filter(kyc_user = user).update(pan_number = kyc_pan_number,id_proof_type = kyc_id_proof_type,
                    #                                                   address_proof_type = kyc_address_proof_type)
                    img_path = kyc_pan_file.path
                    response_kyc = Pan_verification(request,img_path,kyc_pan_number)
                    if response_kyc == 400:
                        messages.error(request,"Images not present in Identity, Update now")
                        return redirect("edit_profile")
        ...
        if bank_qs:
            ...
            form = BankAccountDetails.objects.filter(bank_account_user = user).update(distributors_name_in_bank_account = bank_distributors_name_in_bank_account,
                            bank_name = bank_bank_name,account_number = bank_account_number,ifsc_code = bank_ifsc_code,branch_name = bank_branch_name
                            )
