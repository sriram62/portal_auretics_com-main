...
from .models import RealTimeDetail, RealTimeOrder, RealTimeUser, RealTimeOrderAudit
...
# import os
# os.environ.setdefault("DJANGO_SETTINGS_MODULE", "portal_auretics_com.settings")
#
...
def create_rt_detail(parent, position, pv, bv, dp, month, year, date):
    # print("running super")
    if RealTimeDetail.objects.filter(user_id=parent).exists():
        order_id = RealTimeDetail.objects.filter(user_id=parent).last()
        order_detail_qs = RealTimeDetail.objects.get(id=order_id.id)
    else:
        order_detail_qs = RealTimeDetail.objects.update_or_create(user_id=parent,defaults={'user_id':parent})[0]
...
def add_pv_bv(current_user, pv, bv, dp, attribute, month, year, date):
    '''
    if RealTimeDetail.objects.filter(user_id=current_user).exists():
        user_id = RealTimeDetail.objects.filter(user_id=current_user).last()
        qs = RealTimeDetail.objects.get(id=user_id.id)

    else:
        qs = RealTimeDetail.objects.create(user_id=current_user)
    '''
...
def rt_infinity_calculation(current_user, pv, bv, dp, parameter, month, year, date):
    ...
    if username_of_referral is not None:
        ...
        '''
        if RealTimeDetail.objects.filter(user_id=username_of_referral).exists():
            username = RealTimeDetail.objects.filter(user_id=username_of_referral).last()
            order_detail_qs = RealTimeDetail.objects.get(id=username.id)
        else:
            order_detail_qs = RealTimeDetail.objects.create(user_id=username_of_referral)
        '''
...
# LATEST ORDERS CONCURRENCY MULTIPROCESSING
def latest_orders_concurrency(tasks_to_accomplish):
    # tasks_to_accomplish = tasks_to_accomplish[0]
    # time.sleep(5)
    try:
        task = tasks_to_accomplish.get_nowait()
    except:
        return
    latest_orders = task[0]
    month = task[1]
    year = task[2]
    power_centers_ids = [3, 6, ]
    low_power_pv = 10000
    stock_variance_ids = [592, ]
    pv_factor = 1
    ...
    date = datetime(int(year), int(month), 1, tzinfo=pytz.UTC)
    ...
    for latest_order in reversed(latest_orders):
        ...
        latest_order_audit_qs = ''
        '''
        try:
            latest_order_audit_qs = RealTimeOrderAudit.objects.get(order_id=latest_order,
                                                                   date__month=month,
                                                                   date__year=year)
            latest_order_audit_qs.rt_calculation_done = True
            latest_order_audit_qs.save()
        except RealTimeOrderAudit.DoesNotExist:
            pass
        '''
        ...
        '''
        if RealTimeOrder.objects.filter(last_order=latest_order).exists():
            order_number = RealTimeOrder.objects.filter(last_order=latest_order).latest('-date')
            order_number_qs = RealTimeOrder.objects.get(id=order_number.id)
        else:
            order_number_qs = RealTimeOrder.objects.create(last_order=latest_order)
        order_number_qs.order_completed_till = latest_order
        '''
        ...
        try:
            ...
            user_highest_qualification = title_qualification_calculation_model.objects.filter(
                user=current_user).latest(
            ...
            # If user is not found in title qualification table (i.e. the user is a registered in this month),
            # we will mark him as blue advisor.
        ...
        '''
        if RealTimeDetail.objects.filter(user_id=current_user).exists():
            order_detail = RealTimeDetail.objects.filter(user_id=current_user).last()
            order_detail_for_audit_qs = RealTimeDetail.objects.get(id=order_detail.id)
        else:
            order_detail_for_audit_qs = RealTimeDetail.objects.create(user_id=current_user)
        '''
        ...
        print("current_pv ===>", current_pv)
        print("low_power_pv ===>", low_power_pv)
        ...
        if (latest_order.material_center.id not in power_centers_ids) or (
            ...
            print(
                "0) if latest_order.material_center.id not in power_centers_ids or if latest_order.material_center.id"
                "in power_centers_ids and current_pv <= low_power_pv:"
            )

            # For old user who's ID is green (Case 1),
            # we will move directly to Infinity Calculation and continue this loop.
            if (user_highest_qualification != "Blue Advisor" and is_user_green):
                print("1) if user_highest_qualification != \"Blue Advisor\":")
                # [Flow] Since rt_infinity_calculation adds PV/BV to GPV/GBV, we are adding PV/BV to user's infinity
                # PPV/PBV seperately.
                add_pv_bv(current_user, current_pv, current_bv, current_dp, ["rt_user_infinity", "case_1", ], month, year, date)
                rt_infinity_calculation(current_user, current_pv, current_bv, current_dp, "rt_gv_month", month, year, date)
            ...
            # current_user_pv_till_last_order = Order.objects.filter(date__lt = latest_order.date, paid = True,
            # email = current_user.email,delete=False).exclude(status=8).latest('pk').pv
            try:
            ...
            # If the user has previously became Green in the current month only, i.e. this order is placed after he
            # has achieved min PV for Green. Then this will become our Case 2 and we will carry this PV to infinity
            # model and continue this loop.
            if current_user_pv_till_last_order >= pv_to_become_active:
                print(
                    "2) if current_user_pv_till_last_order != 0 and current_user_pv_till_last_order >= "
                    "pv_to_become_active:")
            ...
            # print("zxzxzxzxzx", (current_user_pv_till_last_order + current_pv) > pv_to_become_active)
            ...
            if (current_user_pv_till_last_order + current_pv) > pv_to_become_active:
                ...
                add_pv_bv(current_user, current_super_pv, current_bv, current_dp,
                          ["rt_user_infinity", "case_3_infinity", ], month,
                          year, date)
                rt_infinity_calculation(current_user, current_super_pv, current_bv,
                                        current_dp, "rt_gv_month", month, year, date)
                ...
                add_pv_bv(current_user, current_pv, current_bv, current_dp, ["case_3_super", ], month, year,
                          date)
        ...
        if super_calculation_required:
            ...
            if need_to_run_infinity_calculation_inside_super:
                ...
                rt_infinity_calculation(current_user, current_pv, 0, current_dp, "rt_gv_month", month, year,
                                        date)
...
def total_pv_bv_calc(latest_orders, month, year, fresh=False):
    if not latest_orders:
        ...
        # return
        date = timezone.now()
        # Pick order excluding orders placed within 1/2 hours
    ...
    date = datetime(int(year), int(month), 1, tzinfo=pytz.UTC) + timedelta(days=1)

    for latest_order in reversed(latest_orders):
        '''
        if RealTimeOrderAudit.objects.filter(order_id=latest_order).exists():
            order_audit = RealTimeOrderAudit.objects.filter(order_id=latest_order).last()
            order_audit_qs = RealTimeOrderAudit.objects.get(id=order_audit.id)
        else:
            order_audit_qs = RealTimeOrderAudit.objects.create(order_id=latest_order)
        '''
    ...
    # threads_to_run = 1 #multiprocessing.cpu_count()
    # print(',,,,,,,,,,,,,,,,,,,,,,,,,',len(latest_orders))
    # chunked_list = list()
    # chunk_size = threads_to_run
    #
    # for i in range(0, len(latest_orders), chunk_size):
    #     chunked_list.append(latest_orders[i:i + chunk_size])
    #
    # number_of_processes = threads_to_run
    # # tasks_to_accomplish = Queue()
    # # tasks_that_are_done = Queue()
    # processes = []
    # for latest_orders in chunked_list:
    #     # number_of_task = len(latest_orders)
    #     tasks_to_accomplish = Queue()
    #     tasks_that_are_done = Queue()
    #     # processes = []
    #
    ...
    #     # data = [[latest_orders[i]], month, year]
    #     data = [[latest_orders[0]], month, year]
    #     tasks_to_accomplish.put(data)
    #
    #     for task in range(number_of_processes):
    #         p = Process(target=latest_orders_concurrency, args=(tasks_to_accomplish,))
    #         processes.append(p)
    #         p.start()
    #
    #     # completing process
    ...
    # AG Code::
    threads_to_run = 2  # multiprocessing.cpu_count()
    # Find number of instances to run
    number_of_processes = threads_to_run

    # Divide the Order in that number of chunks
    no_of_orders_in_a_chunk = float(len(latest_orders)) / float(number_of_processes)
    no_of_orders_in_a_chunk = int(math.floor(no_of_orders_in_a_chunk))
    # We will process balance chunks at the end
    no_of_orders_in_last_chunk = len(latest_orders) - (no_of_orders_in_a_chunk * number_of_processes)
    latest_orders_start = 0
    latest_orders_end = 0
    tasks_to_accomplish = Queue()
    tasks_that_are_done = Queue()
    processes = []
    for i in range(number_of_processes):
        # time.sleep(10)
        if no_of_orders_in_last_chunk > 0:
            if i == range(number_of_processes)[-1]:
                no_of_orders_in_a_chunk = no_of_orders_in_last_chunk
        latest_orders_start = latest_orders_end
        latest_orders_end = latest_orders_start + no_of_orders_in_a_chunk
        orders_to_process = latest_orders[latest_orders_start:latest_orders_end]
        data = [orders_to_process, month, year]
        tasks_to_accomplish.put(data)
        p = Process(target=latest_orders_concurrency, args=(tasks_to_accomplish,))
        processes.append(p)
        p.start()
